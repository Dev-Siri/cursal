...
use crate::{
    args::CursalArgs,
    utils::{is_supported_http_method, is_valid_url, str_to_reqwest_method},
};
use clap::Parser;
use http::send_request;
use reqwest::Method;
use stdout::format_http_response;
use utils::can_send_body;
...
async fn main() -> Result<()> {
    ...
    if body.starts_with("@") {
        ...
        body = fs::read_to_string(path_to_data_file).expect("Failed to read data file");
    }
